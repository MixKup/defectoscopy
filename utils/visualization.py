...
class BBoxVisualization():
    ...
    def __init__(self, cls_dict):
        ...
        self.colors = gen_colors(len(cls_dict)+1)
    ...
    def draw_bboxes(self, img, boxes, confs, clss):
        ...
        for bb, cf, cl in zip(boxes, confs, clss):
            ...
            txt_loc = (max(x_min+1, 0), max(y_min+1, 0))
    ...
    def draw_tracked_dots(self, img, track_boxes):
        for bb in track_boxes:
            track_id = bb[4]
            w, h = bb[2] - bb[0], bb[3] - bb[1]
            radius = 5
            color = (255, 0, 0)
            thickness = 2
            cv2.circle(img, (int(bb[0] + (w/2)), int(bb[1] + (h/2))), radius, color, thickness)
            txt = str(track_id)
            txt_loc = (max(0, int(bb[0])), max(int(bb[1]), 0))
            img = draw_boxed_text(img, txt, txt_loc, color)
            #time.sleep(0.05)
